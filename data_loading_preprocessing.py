...
def parse_csv_dataset(dataset: tf.data.Dataset):
    ...
    for line in dataset.as_numpy_iterator():
        ...
        # Move Price column at the end
        fields[25], fields[42] = fields[42], fields[25]
        # Remove CarId and CarName columns
        new_dataset.append(fields[1] + fields[3:])

    return new_dataset

def add_encoded_name(dataset: list[tf.Tensor]):
    data: list[tf.Tensor]= []
...
def create_dataset(
    ...
    dataset = parse_csv_dataset(dataset)
    dataset = add_encoded_name(dataset).cache()
